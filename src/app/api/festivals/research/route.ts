...
const resolveMaxBatchSize = () => {
    ...
    const min = 1;
    const max = 100;
    const raw = Number.parseInt(process.env.FESTIVAL_RESEARCH_MAX_IDS || process.env.FESTIVAL_NAVER_UPDATE_MAX_IDS || String(fallback), 10);
...
export async function POST(req: NextRequest) {
    ...
    try {
        ...
        console.error("API Festival Research Error:", error);
